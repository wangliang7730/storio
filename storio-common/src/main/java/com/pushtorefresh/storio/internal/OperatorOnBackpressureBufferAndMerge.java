...
public class OperatorOnBackpressureBufferAndMerge<T> implements Operator<T, T> {

    private final Long capacity;
    private final Action0 onOverflow;

    private static class Holder {
        static final OperatorOnBackpressureBufferAndMerge<?> INSTANCE = new OperatorOnBackpressureBufferAndMerge<Object>();
    }
    
    @SuppressWarnings("unchecked")
    public static <T> OperatorOnBackpressureBufferAndMerge<T> instance() {
        return (OperatorOnBackpressureBufferAndMerge<T>) Holder.INSTANCE;
    }
    
    private OperatorOnBackpressureBufferAndMerge() {
        this.capacity = null;
        this.onOverflow = null;
    }

    public OperatorOnBackpressureBufferAndMerge(long capacity) {
        this(capacity, null);
    }

    public OperatorOnBackpressureBufferAndMerge(long capacity, Action0 onOverflow) {
        if (capacity <= 0) {
    ...
    private static final class BufferSubscriber<T> extends Subscriber<T> implements BackpressureDrainManager.BackpressureQueueCallback {
        // TODO DataStructures.filter(isQueue && isSet && isConcurrent) -> ConcurrentLinkedHashSet
        private final ConcurrentLinkedQueue<Object> queue = new ConcurrentLinkedQueue<Object>();
        private final Set<Object> set = Collections.newSetFromMap(new ConcurrentHashMap<Object, Boolean>());
